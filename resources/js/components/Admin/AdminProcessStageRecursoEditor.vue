...
function openEditor(stage: StageRow): void {
    ...
    stageForm.recurso_inicio_em = toDatetimeLocalInputValue(
    ...
    >
        <form v-if="editingStage" class="space-y-4" @submit.prevent="submit">
            <p class="text-sm text-muted-foreground">
                Se não informar as datas, o sistema usa automaticamente até 5
                dias após o fim da etapa (<span v-if="editingStage.fim_em">{{
                    formatPeriod(null, editingStage.fim_em)
                }}</span
                ><span v-else>defina o fim da etapa</span>).
            </p>
            <label class="flex flex-col gap-1.5">
                <span class="text-sm"
                    >Início do prazo de recurso (opcional)</span
                >
                <InputText
