...
const avatarSrc = computed<string | null>(() =>
    resolveUserAvatarUrl(props.user),
);
const initials = computed(() => getInitials(displayName.value));
const isEmailVerified = computed(() =>
    Boolean(asText(props.user?.email_verified_at)),
);
</script>

<template>
    <div
        class="relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-background via-card to-muted/40 px-5 py-5 shadow-sm sm:px-7 sm:py-6 dark:border-border/40 dark:from-card dark:via-card dark:to-muted/15"
    >
        <div
            class="pointer-events-none absolute -top-24 -right-20 size-72 rounded-full bg-primary/[0.07] blur-3xl"
            aria-hidden="true"
        />
        <div
        ...
        >
            <div class="flex min-w-0 flex-1 items-center gap-4 sm:gap-5">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div
                        class="mt-2 flex flex-wrap items-center gap-1.5 text-[12px]"
                    >
                        <span
                            v-if="cpfMasked"
                            v-tooltip.bottom="
                                'CPF parcialmente mascarado por privacidade'
                            "
                    ...
                    <p
                        v-if="lastUpdate"
                        class="mt-2 text-[11.5px] text-muted-foreground"
                    >
                        Atualizado
            ...
            <div
                class="flex w-full shrink-0 items-center gap-2 sm:w-auto sm:flex-col sm:items-end sm:gap-2.5"
            >
                <Button
                ...
                >
                    Clique em editar para alterar seus dados sem sair desta
                    inscrição.
                </p>
            </div>
        </div>
