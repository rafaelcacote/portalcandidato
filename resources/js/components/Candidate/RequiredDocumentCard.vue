...
<template>
    ...
    >
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="text-sm font-medium text-foreground">{{ doc.nome }}</span>
                    <Tag v-if="doc.obrigatorio" value="Obrigatório" severity="danger" class="text-xs" />
                    <Tag v-else value="Opcional" severity="secondary" class="text-xs" />
                </div>
                <p v-if="doc.descricao" class="mt-1 text-xs text-muted-foreground">
                    {{ doc.descricao }}
                </p>
            </div>
            ...
            <div class="shrink-0">
                ...
                />
                <Tag v-else value="Pendente" severity="warn" icon="pi pi-clock" />
            </div>
        </div>
        ...
        <div v-if="!isFinalized" class="mt-3">
            <input ref="fileInputRef" type="file" class="sr-only" @change="onFileChange" />
