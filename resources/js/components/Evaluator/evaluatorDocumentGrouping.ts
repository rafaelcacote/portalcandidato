...
function sortById(
    a: EvaluatorApplicationDocument,
    b: EvaluatorApplicationDocument,
): number {
    return a.id - b.id;
}

function titleItemOf(
    doc: EvaluatorApplicationDocument,
): EvaluatorApplicationDocument['title_item'] {
    return doc.title_item ?? null;
}

function sortTitleDocuments(
    docs: EvaluatorApplicationDocument[],
): EvaluatorApplicationDocument[] {
...
export function buildEvaluatorDocumentSections(
    ...
    const required = documents.filter(
        (d) =>
            d.required_document != null ||
            (d.process_required_document_id != null &&
                d.process_required_document_id > 0),
    );
    ...
    const titleLinked = documents.filter(
    ...
    if (orphanTitles.length > 0) {
        sections.push({
            ...
            description:
                'Anexos da pontuação de títulos (sem grupo cadastrado ou metadados incompletos).',
            kind: 'titles',
    ...
    if (special.length > 0) {
        sections.push({
            ...
            description:
                'Anexos especiais da inscrição (ex.: deficiência, declarações).',
            kind: 'special',
    ...
    if (other.length > 0) {
        sections.push({
            ...
            title: 'Demais anexos',
            description:
                'Itens que não se encaixaram nas categorias acima — valide normalmente.',
            kind: 'other',
