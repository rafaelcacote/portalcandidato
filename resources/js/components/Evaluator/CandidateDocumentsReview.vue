...
const sections = computed(() => buildEvaluatorDocumentSections(props.documents ?? []));
...
const globalStats = computed(() => {
    ...
    const pending = docs.filter((d) => d.status !== 'aprovado' && d.status !== 'recusado').length;

    return { total, approved, refused, pending };
});

function sectionStats(docs: EvaluatorApplicationDocument[]): { approved: number; refused: number; pending: number } {
    const approved = docs.filter((d) => d.status === 'aprovado').length;
    const refused = docs.filter((d) => d.status === 'recusado').length;
    const pending = docs.filter((d) => d.status !== 'aprovado' && d.status !== 'recusado').length;
...
<template>
    <div class="rounded-2xl bg-white ring-1 ring-slate-200/80 shadow-sm">
        <!-- Header -->
        <div class="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 sm:px-6 sm:py-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="flex gap-3">
                    <div class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-600">
                        <FileCheck class="size-5" />
                    </div>
                    <div>
                        <h3 class="text-base font-bold tracking-tight text-slate-900 sm:text-lg">
                            Validação de documentos
                        </h3>
                        <p class="mt-1 max-w-2xl text-sm leading-relaxed text-slate-500">
                            Os anexos estão organizados por tipo: primeiro a documentação obrigatória do edital, depois os
                            <strong class="font-medium text-slate-600">títulos para pontuação</strong>
                            (agrupados como no processo) e, por fim, complementos como PcD. Use cada bloco como um checklist.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 lg:shrink-0 lg:justify-end">
                    ...
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200/70">
                        <CheckCircle2 class="size-3" />
                        {{ globalStats.approved }}
                    </span>
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-700 ring-1 ring-red-200/70">
                        <XCircle class="size-3" />
                        {{ globalStats.refused }}
                    </span>
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800 ring-1 ring-amber-200/70">
                        <Clock class="size-3" />
                        {{ globalStats.pending }}
                    </span>
                </div>
        ...
        <div class="flex flex-col gap-4 p-4 sm:p-5">
            ...
            >
                ...
                >
                    ...
                    >
                        <component :is="sectionIcon(section.kind)" class="size-4.5" />
                    </div>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600">
                                ...
                                {{ section.documents.length === 1 ? 'item' : 'itens' }}
                            </span>
                            <span
                                v-if="sectionStats(section.documents).pending > 0"
                                class="rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-800 ring-1 ring-amber-200/70"
                            >
                                {{ sectionStats(section.documents).pending }} pendente{{ sectionStats(section.documents).pending !== 1 ? 's' : '' }}
                            </span>
                        </div>
                        <p v-if="section.description" class="mt-1 text-xs leading-relaxed text-slate-500 sm:text-sm">
                            {{ section.description }}
                        </p>
                    </div>
                    ...
                    <div class="hidden shrink-0 items-center gap-2 sm:flex">
                        ...
                        <span class="inline-flex items-center gap-1 text-[11px] font-medium text-red-600">
                ...
                <div class="border-t border-slate-200/60 bg-white px-3 py-3 sm:px-4 sm:py-4">
            ...
            >
                ...
                <p class="mt-3 text-sm font-semibold text-slate-600">Nenhum documento enviado</p>
