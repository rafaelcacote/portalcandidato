...
const emit = defineEmits<{
    'update:scores': [value: Array<{ process_criteria_id: number; pontuacao: number }>];
...
const totalScore = computed(() => {
    const criteriaPart = props.scores.reduce((sum, s) => sum + (Number(s.pontuacao) || 0), 0);
    const titlePart = Number(props.titleScoringCurrent ?? 0);
    return criteriaPart + titlePart;
});

const maxScore = computed(() => {
    const criteriaPart = props.criteria.reduce((sum, c) => sum + c.pontuacao_max, 0);
    ...
    <div class="rounded-2xl bg-white ring-1 ring-slate-200/80 shadow-sm">
        ...
        <div class="flex flex-col gap-3 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-900">{{ sectionHeading }}</h3>
            ...
            <!-- Total score badge -->
            <div class="flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-50 to-indigo-50 px-4 py-2 ring-1 ring-violet-200/60">
                <Trophy class="size-4 text-violet-500" />
                <span class="text-xs font-semibold text-slate-500">Pontuação total</span>
                <span :class="['text-lg font-bold tabular-nums', scoreLabelColor]">
        ...
        <div class="px-6 pt-4 pb-2">
            <div class="flex items-center justify-between gap-3">
                <div class="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                    <div
                        :class="['h-full rounded-full transition-all duration-500', scoreColor]"
                        :style="{ width: `${scorePercentage}%` }"
                    />
                </div>
                <span :class="['shrink-0 text-sm font-bold tabular-nums', scoreLabelColor]">
                    {{ scorePercentage }}%
                </span>
            </div>
        </div>
            ...
            >
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        <p class="text-sm font-semibold text-slate-800">{{ criterion.nome }}</p>
                        <span class="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-500">
                            Peso {{ criterion.peso }}
                        </span>
                    </div>
                </div>
                ...
                <div class="flex shrink-0 items-center gap-3">
                    <span class="text-xs font-medium text-slate-400">
                        Máx. <strong class="text-slate-600">{{ criterion.pontuacao_max }}</strong>
                    </span>
                    <div class="relative">
                        <input
                            type="number"
                            ...
                            step="0.5"
                            class="w-24 rounded-xl border border-slate-200 bg-white px-3 py-2 text-center text-sm font-bold text-slate-800 outline-none transition-colors focus:border-violet-400 focus:ring-2 focus:ring-violet-400/20"
                            @input="(e) => updateScore(index, parseFloat((e.target as HTMLInputElement).value) || 0)"
                        />
                    </div>
        ...
        <div class="border-t border-slate-100 px-6 py-5">
            ...
            <textarea
                ...
                class="w-full resize-none rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm text-slate-700 placeholder-slate-400 outline-none transition-colors focus:border-violet-400 focus:bg-white focus:ring-2 focus:ring-violet-400/20"
                @input="(e) => emit('update:observacoes', (e.target as HTMLTextAreaElement).value)"
            />
        </div>
    </div>
</template>
