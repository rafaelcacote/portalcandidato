...
const form = useForm({
    status: props.document.status !== 'pendente' ? props.document.status : 'pendente',
...
<template>
    ...
    >
            ...
            <div class="flex min-w-0 flex-1 gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        <span
                            v-if="contextBadge"
                            class="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-500"
                        >
                            {{ contextBadge }}
                        </span>
                        <div class="sm:hidden">
                    ...
                    <h5 class="mt-1 text-sm font-semibold leading-snug text-slate-900 sm:text-base">
                        {{ heading }}
                    </h5>
                    <p v-if="subheading" class="mt-1 text-xs leading-relaxed text-slate-500 sm:text-sm">
                        {{ subheading }}
                    </p>
                    <p class="mt-2 font-mono text-[11px] text-slate-400 sm:text-xs">
                        <span class="font-sans font-medium text-slate-400">Arquivo:</span>
                        {{ document.nome_arquivo }}
                    </p>
                </div>
            </div>

            <div class="flex shrink-0 flex-row items-center justify-between gap-3 sm:flex-col sm:items-end">
        ...
        <div class="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 lg:flex-row lg:items-end lg:gap-4">
            <div class="min-w-0 flex-1">
                <label class="mb-1.5 block text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                ...
                <textarea
                    ...
                    class="w-full resize-y rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:border-teal-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-400/20"
                />
            </div>
            <div class="flex shrink-0 items-center justify-end gap-2 lg:flex-col lg:items-stretch">
                <span class="mb-0 mr-auto text-[11px] font-medium text-slate-400 lg:hidden">Decisão</span>
                <div class="flex gap-2">
                    <button
                        ...
                        :class="[
                            'flex size-10 items-center justify-center rounded-xl transition-all duration-150 focus-visible:outline-none focus-visible:ring-2',
